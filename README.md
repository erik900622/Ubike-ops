...
- 當某站點可借車輛少於 1 台時，系統自動標記為「空車風險」，提醒補車
- 當某站點可還車位少於 1 個時，系統標記為「滿車風險」，提醒移車
- 各行政區在 06–22 時段有不同尖峰 → 透過熱力圖清楚辨識補車優先順序
- 站點在未來 0–60 分鐘可能持續下降 → 預測模型提前警示
...
- 分析最近 30 筆資料點
- 使用線性局部斜率平均（比單純線性回歸更穩定）
- 結果做 capacity clip（避免超出容量）
- 預測未來需求並標示風險等級
